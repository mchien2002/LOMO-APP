...
class WheelListPickerWidget extends StatelessWidget {
  ...
  WheelListPickerWidget(
      {required this.items, this.initItemIndex, this.onSelectedItemChanged});

  @override
  Widget build(BuildContext context) {
    return CupertinoPicker(
      scrollController:
          FixedExtentScrollController(initialItem: initItemIndex ?? 0),
      itemExtent: 50,
      ...
      children: items
          .map((item) => Center(
                child: Text(
                  item,
                  style: textTheme(context).subText.colorPrimary,
  ...
  Widget build(BuildContext context) {
    return ListView.builder(
        ...
        controller: ScrollController(
            initialScrollOffset:
                widget.initItemIndex != -1 && widget.initItemIndex != 0
                    ? widget.initItemIndex * 51.0 - 22
                    : 0.0),
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: widget.initItemIndex == index
              ? getColor().colorVioletEB
              : getColor().white,
          border: Border(
              bottom: BorderSide(
                  color: getColor().underlineClearTextField, width: 0.75)),
        ),
        child: Text(
          item,
          style: widget.initItemIndex == index
              ? textTheme(context).text14Normal.colorViolet
              : textTheme(context).text14Normal.colorDart,
        ),
      ),
    );
  }
...
class SogiescsListPickerWidget extends StatefulWidget {
  ...
  @override
  _SogiescsListPickerWidgetState createState() =>
      _SogiescsListPickerWidgetState();
}
...
class _SogiescsListPickerWidgetState extends State<SogiescsListPickerWidget> {
  ...
  void initState() {
    super.initState();
    if (widget.initItemIndex?.isNotEmpty == true)
      listInitial = List.from(widget.initItemIndex!);
  }
  ...
  Widget build(BuildContext context) {
    return ListView.builder(
        itemCount: widget.items.length,
        controller: ScrollController(
            initialScrollOffset: listInitial.isNotEmpty == true
                ? widget.items.indexOf(listInitial[0]) * 51.0 - 22
                : 0.0),
        itemBuilder: (context, index) {
          return _buildItem(widget.items[index], index, context);
        });
  ...
  Widget _buildItem(String item, int index, BuildContext context) {
    return InkWell(
      ...
      child: Container(
        ...
        alignment: Alignment.center,
        decoration: BoxDecoration(
          color: listInitial.contains(item)
              ? getColor().colorVioletEB
              : getColor().white,
          border: Border(
              bottom: BorderSide(
        ...
        child: Text(
          item,
          style: listInitial.contains(item)
              ? textTheme(context).text14Normal.colorViolet
              : textTheme(context).text14Normal.colorDart,
        ),
      ),
