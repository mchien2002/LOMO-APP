...
import 'annotation_editing_controller.dart';
...
class FlutterMentionsState extends State<FlutterMentions> {
  ...
  Map<String, Annotation> mapToAnotation() {
    ...
    widget.mentions?.forEach((element) {
      ...
      if (element.matchAll!) {
        data['${element.trigger}${element.regex}'] = Annotation(
          style: element.style,
          id: null,
          display: null,
  ...
  void addMention(Map<String, dynamic> value, [Mention? list]) {
    ...
    final _list = selectedMention != null
        ? widget.mentions!.firstWhere((element) =>
            selectedMention.str != null &&
            selectedMention.str!.contains(element.trigger!))
        : widget.mentions![0];
    ...
    int lengthOfTagged = value['display']?.length;
    int nextCursorPosition = ((selectedMention.start! + 1) + lengthOfTagged);
